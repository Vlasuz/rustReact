...
export const BattleMode1V1: React.FC<IBattleMode1V1Props> = ({itemData}) => {
    ...
    const joinToGame = (position: number) => {
        getBearer({type: "post"})
        axios.post(getApiLink(`api/battle/join/?game_id=${itemData.id}&position=${position}`)).then(({data}) => {
            console.log(data)
            navigate(`/battle/${itemData?.id}`)
            setIsJoinToGame(true)
        }).catch(er => console.log(getApiLink(`api/battle/join/?game_id=${itemData.id}&position=${position}`), er))
    }

    const isWinnerPosition1 = itemData?.winners?.length && itemData.winners[0].position === 1

    return (
        <div className="players">
            <div className={`player player_blue ${!isWinnerPosition1 && "player_looser"}`}>
                <div className="player__photo">
                    <img src={itemData?.players[0]?.user?.avatar} alt="Photo"/>
                </div>
            </div>
            <b>VS</b>
            <div className={`player player_green ${isWinnerPosition1 && "player_looser"}`}>

                {itemData?.players[1]?.user ? <div className="player__photo">
                        <img src={itemData?.players[1]?.user?.avatar} alt="Photo"/>
                    </div> :
                    <button onClick={_ => joinToGame(2)} className="player__photo player__photo_loading">
