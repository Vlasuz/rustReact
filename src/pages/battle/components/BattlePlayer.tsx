...
const ButtonWithoutPlayer = ({joinToGame}: any) => {
...
export const BattlePlayer: React.FC<IBattlePlayerProps> = ({isWinner, numberOfPosition, itemData, isGroup, color}) => {
    ...
    const player = itemData?.players.filter(item => item.position === numberOfPosition)[0]
    const isHavePlayerOnThisPosition = itemData?.players.some(item => item.position === numberOfPosition)
